...
#define RTC_SLOT  65   // Avoid slot 0 (used by core)
...
// ===== Telegram / WiFi connection guards =====
//bool telegramTriggeredForThisConnection = false;
/*
bool telegramSendPending = false;
bool telegramSending = false;
...
bool isMidCycleWake = false;        // True if waking from chunked sleep
//bool resumeSleepAfterBoot = false;  // Flag to resume sleep
//bool webServerStartPending = false;
bool serverRunning = false;
...
*/void applyEstimatedTimeFromSnapshot() {
  ...
  if (!f) {
    Serial.println(F("  No saved time snapshot found."));
    return;
  }
...
void setup() {
    Serial.begin(115200);
    Serial.println(F("\n\n"));
    Serial.println(F("ESP8266 core detected!"));
    ...
    if (rtcValid) {
        ...
        if (rtcData.remainingSleepSeconds > 86400UL) {
            Serial.println(F(" RTC corrupted (huge value). Clearing."));
            clearRTC();
            rtcValid = false;
        }
    } else {
    ...
    if (rtcValid && rtcData.remainingSleepSeconds > 0) {
        ...
        isMidCycleWake = true;
        Serial.println(F("===== MINIMAL SETUP (mid-cycle) ====="));
        return;  // Skip everything else, loop() will handle sleep
    }
    
    // ========== FINAL WAKE - FULL INITIALIZATION ==========
    ...
    if (!LittleFS.begin()) {
        ...
        if (!LittleFS.check()) {
            Serial.println(F(" LittleFS consistency check failed!"));
        } else {
            Serial.println(F("✅ LittleFS check passed."));
        }
    }
    ...
    if (!loadBootRecovery()) {
        Serial.println(F(" Boot recovery failed."));
        factoryResetPreserveIndex();
    }
    ...
    // ========== RESET ALL FLAGS ==========
    Serial.println(F("Resetting all state flags..."));
...
void manageWiFiConnection() {
    ...
    // ========== SKIP IF MID-CYCLE WAKE ==========
    if (isMidCycleWake) {
        return;  // Don't manage WiFi during mid-cycle sleep
    }
    ...
    if (status != WL_CONNECTED) {
        ...
        // 🚫 Do NOT spam WiFi.begin while connecting
            ...
            if (!tryingHardcodedNetworks) {
                ...
                if (millis() - hardcodedConnectAttemptStart >= HARDCODED_CONNECT_TIMEOUT_MS) {
                    ...
                    if (currentHardcodedNetworkIndex < NUM_HARDCODED_NETWORKS) {
                        ...
                        // 🚫 Do NOT enter AP mode while Telegram might be needed
    ...
    if (!wifiConnected) {
        ...
        // ========== TELEGRAM TRIGGER DEBUG ==========
        Serial.println(F("========== TELEGRAM TRIGGER DEBUG =========="));
        Serial.printf("telegramTriggeredForThisConnection = %d\n", telegramTriggeredForThisConnection);
        Serial.printf("telegramSendPending = %d\n", telegramSendPending);
        Serial.printf("telegramDelayActive = %d\n", telegramDelayActive);
        Serial.printf("telegramSending = %d\n", telegramSending);
        Serial.printf("isMidCycleWake = %d\n", isMidCycleWake);
        Serial.printf("rtcData.remainingSleepSeconds = %u\n", rtcData.remainingSleepSeconds);
        Serial.println(F("============================================="));
        ...
        if (!telegramTriggeredForThisConnection) {
            ...
            Serial.printf("telegramSendPending is now: %d\n", telegramSendPending);
        } else {
            Serial.println(F(" Telegram already triggered, skipping"));
        }

        // 💾 Save credentials if SSID changed
    ...
    // ✅ Update NTP ONLY when connected
    if (WiFi.status() == WL_CONNECTED) {
        timeClient.update();
    }
}
...
void manageWiFiConnection() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        if (wifiConnected) {
            ...
            // 🔄 Reset telegram state on disconnect
            telegramSendPending = false;
            telegramDelayActive = false;
            telegramSending = false;
        }
    ...
    if (!wifiConnected) {
        ...
        // 📢 ALWAYS arm Telegram after STA connect
        telegramSendPending = true;
        telegramDelayActive = false;
        telegramSending = false;
        telegramRetryCount = 0;

        Serial.println(F("Telegram armed after WiFi connect"));
...
/*void manageWiFiConnection() {
    ...
    if (!wifiConnected) {
        ...
        // 📢 ALWAYS arm Telegram on connection
        telegramSendPending = true;
        telegramDelayActive = false;
        telegramSending = false;
        telegramRetryCount = 0;
...
void manageWiFiConnection() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        // 🔑 ONLY reset once on real disconnect
        if (wifiConnected) {
            wifiConnected = false;
            telegramTriggeredForThisConnection = false;
        }
    ...
    if (!wifiConnected) {
        ...
        // 📢 Telegram trigger (ONCE per real connection)
if (!telegramTriggeredForThisConnection) {
    ...
    telegramRetryCount = 0;          // 🔴 REQUIRED
    telegramAttemptFinished = false; // 🔴 REQUIRED

    webStartAt = 0;                  // 🔴 REQUIRED

    telegramTriggeredForThisConnection = true;

    Serial.println(F("Telegram armed for this WiFi connection"));
}
...
void loop() {
    ...
    // ========================================
    // 🔴 SECTION 1: MID-CYCLE WAKE - SLEEP IMMEDIATELY
    // ========================================
    if (isMidCycleWake && rtcData.remainingSleepSeconds > 0) {
        static unsigned long midCycleStart = 0;
        if (midCycleStart == 0) midCycleStart = millis();
        
        if (millis() - midCycleStart > 500) {
            ...
            Serial.printf("Sleeping %u sec\n", sleepNow);
            WiFi.mode(WIFI_OFF);
            delay(100);
            ESP.deepSleep(sleepNow * 1000000UL);
        }
        return;  // Don't run anything else during mid-cycle
    }
    ...
    if (millis() - lastDebug > 3000) {
        ...
        Serial.printf("Telegram - pending:%d delay:%d sending:%d triggered:%d\n", 
                      telegramSendPending, telegramDelayActive, 
                      telegramSending, telegramTriggeredForThisConnection);
        Serial.printf("WebServer - pending:%d running:%d\n", 
                      webServerStartPending, serverRunning);
        Serial.printf("deepSleepRequested: %d\n", deepSleepRequested);
    ...
    // ========================================
    // 📱 SECTION 3: TELEGRAM STATE MACHINE
    // ========================================
    
    // Step 1: Pending -> Start Delay
    if (telegramSendPending && !telegramDelayActive && !telegramSending) {
        Serial.println(F(" TELEGRAM STATE: Starting 3-second delay..."));
        telegramDelayActive = true;
        telegramSendStartTime = millis();
    }

    // Step 2: Check if delay is complete
    if (telegramDelayActive && !telegramSending) {
        ...
        if (millis() - lastProgressPrint > 1000) {
            ...
            Serial.printf(" TELEGRAM: Waiting... %lu/3000 ms\n", elapsed);
        }
        
        if (elapsed >= 3000) {
            Serial.println(F(" TELEGRAM STATE: Delay complete, SENDING NOW!"));
            telegramDelayActive = false;
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                Serial.println(F(" Calling bot.sendMessage()..."));
                ...
                Serial.printf(" TELEGRAM RESULT: %s (took %lu ms)\n", 
                              success ? "✅ SUCCESS" : "❌ FAILED", sendDuration);
                
                if (!success && telegramRetryCount < 2) {
                    telegramRetryCount++;
                    Serial.printf(" Retry %d/2...\n", telegramRetryCount);
                    telegramDelayActive = true;
                    telegramSendStartTime = millis();
                    telegramSending = false;
                    return;
                ...
                Serial.println(F("❌ WiFi disconnected, cannot send Telegram"));
            }
            ...
            Serial.println(F(" TELEGRAM: Complete! webServerStartPending = true"));
        }
    }
    ...
    if (webServerStartPending && !serverRunning) {
        ...
        serverRunning = true;
        webServerStartPending = false;
        Serial.println(F("✅ Web server is now running!"));
    }

    // ========================================
    // 💤 SECTION 5: USER-REQUESTED DEEP SLEEP
    // ========================================
    
    if (deepSleepRequested) {
        // Wait for Telegram to finish
        if (telegramSendPending || telegramDelayActive || telegramSending) {
            Serial.println(F("⏳ Waiting for Telegram before sleep..."));
            return;
        }
        ...
        if (millis() - sleepRequestTime > 500) {
            ...
            uint32_t sleepNow = settings.deepSleepDurationSeconds;

            if (sleepNow == 0) {
                Serial.println(F(" Sleep duration is 0, aborting"));
                deepSleepRequested = false;
                sleepRequestTime = 0;
                return;
            }
    ...
    if (relayTimedOffPending) {
        ...
        if (relayState) {
            ...
            Serial.printf("Timed Relay OFF after %lu seconds.\n", actualDuration);
            ...
            if (settings.autoDeepSleepEnabled && !autoDeepSleepScheduled && 
                ...
                if (!isTemporaryRun || actualDuration >= 600) {
                    ...
                    Serial.printf("Scheduling auto deep sleep in %lu minutes.\n",
...
void checkAPTimeout() {
  ...
  const unsigned long AP_CLIENT_TIMEOUT_MS    = 14UL * 60UL * 1000UL; // 14 minutes
...
void handleDeepSleep(AsyncWebServerRequest *request) {
  ...
  initiateAutoDeepSleep();   // 🔑 CORRECT ENTRY POINT

  request->send(200, "text/plain", "Initiating deep sleep.");
  // Deep sleep will occur in loop()
}
void handleFormatFS(AsyncWebServerRequest *request) {
  if (!isLoggedIn(request)) {
    request->send(401, "text/plain", "Unauthorized");
...
void handleFileUpload(AsyncWebServerRequest *request,
  ...
  String filename_str = filename; // optional rename
