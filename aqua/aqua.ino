...
// Now you can use hardcodedNetworks
#include <Arduino.h>
#include <LittleFS.h>
...
void setup() {
  ...
  Serial.printf("Reset reason: %s\n", ESP.getResetReason().c_str());
    ...
    // Initialize the improved WiFi Manager ← ADD THIS
    initWiFiManager();
    
    // Enable hunt mode for intermittent WiFi
...
//  initWiFi();
  ...
  Serial.println(F("✓ Setup complete. Hunting for WiFi...\n"));
}
...
void loop() {
    ...
    if (millis() - lastDebug > 3000) {
        ...
        Serial.printf("WiFi: %s | Status: %d | Connected: %d\n", 
                      getWiFiStateName(), WiFi.status(), wifiConnected);
        Serial.printf("Mode: %s | Scans: %d\n", 
                      isInHuntMode() ? "HUNT" : "NORMAL", getTotalScanCount());
    ...
    if (telegramSendPending && !telegramDelayActive && !telegramSending) {
        ...
        // HUNT MODE: Send immediately
        if (isInHuntMode() && isWiFiConnected()) {
            Serial.println(F(" HUNT: Sending Telegram immediately..."));
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                for (int attempt = 0; attempt < 2 && !success; attempt++) {
                    ...
                    success = bot.sendMessage(CHAT_ID, ipMsg, "");
                ...
                if (success) {
                    ...
                    Serial.println(F("❌ Failed - retry next connection"));
                }
            }
        ...
        // NORMAL MODE: Use delay
        else if (!isInHuntMode()) {
            Serial.println(F(" Starting 3-second delay..."));
            telegramDelayActive = true;
            telegramSendStartTime = millis();
        }
    }

    // Normal mode: Wait for delay then send
    if (telegramDelayActive && !telegramSending && !isInHuntMode()) {
    ...
    if (webServerStartPending && !serverRunning) {
        if (isInHuntMode() && !isWiFiConnected()) {
            // Wait for connection in hunt mode
        } else {
    ...
    if (deepSleepRequested) {
        ...
        if (millis() - sleepRequestTime > 500) {
            ...
            if (totalSleep == 0) {
                Serial.println(F("⚠️ Sleep duration 0, abort"));
                deepSleepRequested = false;
                sleepRequestTime = 0;
                return;
            }
    ...
    if (!postConnectScanDone && WiFi.status() == WL_CONNECTED && !scanInProgress) {
        Serial.println(F(" Post-connection scan..."));
    ...
    static uint32_t lastWiFiStatus = 0;
    if (millis() - lastWiFiStatus >= 30000) {
        printWiFiManagerStatus();
    ...
    if (millis() - lastStatusPrint >= 10000) {
        Serial.print(F(" WiFi: "));
        Serial.print(getWiFiStateName());
        Serial.print(F(" | "));
        Serial.print(wifiConnected ? "Connected" : "Disconnected");
        if (wifiConnected) {
            Serial.printf(" | RSSI: %ddBm", getCurrentRSSI());
        }
        if (isInHuntMode()) {
            Serial.printf(" | Scans: %d", getTotalScanCount());
        }
        // Show failure counters if any
        if (getWrongPasswordCount() > 0 || getPoorSignalDisconnectCount() > 0) {
            Serial.printf(" | PW:%d PS:%d", 
                          getWrongPasswordCount(), 
                          getPoorSignalDisconnectCount());
        }
        Serial.println();
    ...
    if (relayTimedOffPending) {
        ...
        if (relayState) {
            ...
            if (settings.autoDeepSleepEnabled && !autoDeepSleepScheduled && 
                ...
                autoDeepSleepScheduled = true;
                Serial.printf("Auto deep sleep in %lu min\n", DEEP_SLEEP_DELAY_MS / 60000);
            }
        }
    }
