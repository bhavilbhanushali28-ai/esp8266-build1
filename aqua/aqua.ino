...
if (telegramDelayActive &&
    ...
    telegramDelayActive = false;
    telegramSending = true;   // 🔒 LOCK IMMEDIATELY
    ...
    telegramSending = false;  // 🔓 UNLOCK

    if (ok) {
    telegramIpSent = true;
    webStartPending = true;   // request web server start
} else {
        telegramSendPending = true;
    }
}
if (webStartPending && !serverRunning) {
    if (webStartAt == 0) {
        webStartAt = millis();   // mark time
    }

    if (millis() - webStartAt >= 1500) {
        startWebServer();
        webStartPending = false;
        webStartAt = 0;
    }
}*/
// ===================== TELEGRAM DELAY ARM =====================
...
if (telegramDelayActive &&
    ...
    // If WiFi dropped, count as failed attempt
    if (WiFi.status() != WL_CONNECTED) {
        telegramSending = false;
        telegramRetryCount++;
        telegramSendPending = (telegramRetryCount < MAX_TELEGRAM_RETRIES);
    } else {
        ...
        // ----- Web server must start regardless -----
        if (!webStartScheduled) {
            webStartPending = true;
            webStartScheduled = true;
        }

        // ----- Retry only up to limit -----
        if (!ok && telegramRetryCount < MAX_TELEGRAM_RETRIES) {
            telegramSendPending = true;
        }
    }
}

// ===================== WEB SERVER START (NON-BLOCKING) =====================
if (webStartPending && !serverRunning) {
