...
#define CHAT_ID   "-1003543724337"                            // Your chat ID
...
void setup() {
  ...
  if (rtcValid && rtcData.remainingSleepSeconds > 0) {
    ...
    isMidCycleWake = true;
    // Don't do any other initialization - just sleep again in loop()
  } else {
  ...
  // ========== SKIP MOST INIT FOR MID-CYCLE WAKE ==========
  if (isMidCycleWake) {
    Serial.println(F("===== MID-CYCLE: Minimal setup ====="));
    // Only need to sleep again, skip everything else
    return;
  }

  // ========== FULL INITIALIZATION (FINAL WAKE ONLY) ==========
  ...
  telegramTriggeredForThisConnection = false;  // ADD THIS!
  
  webServerStartPending = false;
  serverRunning = false;
  wifiConnected = false;  // ADD THIS!
...
void manageWiFiConnection() {
    ...
    if (!wifiConnected) {
        ...
        if (!telegramTriggeredForThisConnection) {
            ...
            Serial.println(F("⚠️ Telegram already triggered, skipping"));
        }
...
void loop() {
  ...
  // ========================================
  // 🔴 SECTION 1: HANDLE MID-CYCLE WAKE FIRST
  // ========================================
  // If this is a mid-cycle wake (from chunked sleep), go back to sleep immediately
  ...
  if (isMidCycleWake && rtcData.remainingSleepSeconds > 0) {
    ...
    if (millis() - midCycleBootTime > 500) {  // 500ms delay
    ...
    return;  // Don't execute anything else during mid-cycle wake
  }

  // ========================================
  // 🟢 SECTION 2: NORMAL OPERATION (FINAL WAKE)
  // ========================================
  // Only reaches here if isMidCycleWake == false OR remainingSleepSeconds == 0
  ...
  // Step 1: If pending, start the delay
  if (telegramSendPending && !telegramDelayActive && !telegramSending) {
  ...
  // Step 2: Wait for delay to complete, then send
  if (telegramDelayActive && !telegramSending) {
    ...
    if (elapsed >= 3000) {
      ...
      // Always complete the state machine
      telegramSending = false;
      telegramSendPending = false;
  ...
  // ========================================
  // This is for when deep sleep is requested AFTER normal operation
  // (e.g., after a relay run completes)
  
  if (deepSleepRequested) {
    // Wait for Telegram to finish first
    if (telegramSendPending || telegramDelayActive || telegramSending) {
      return;
    }
  ...
  // WiFi scan (once after connection)
  processScanResults();
  ...
  if (relayTimedOffPending) {
    ...
    if (relayState) {
      ...
      // Schedule auto deep sleep if enabled
