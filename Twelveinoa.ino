  ...
  if (dnsServerActive) {
    ...
    if (telegramDelayActive &&
        ...
        if (WiFi.status() == WL_CONNECTED && WiFi.localIP()[0] != 0) {
            ...
            ESP.wdtFeed();
            ok = bot.sendMessage(CHAT_ID, ipMsg, "");
            ESP.wdtFeed();
        }
        ...
        if (ok) {
            telegramIpSent = true;
        } else if (telegramRetryCount < MAX_TELEGRAM_RETRIES) {
            telegramSendPending = true;
        }

        // ✅ Telegram attempt finished (success OR fail)
        telegramAttemptFinished = true;
    }

    // ===================== WEB SERVER START =====================
    if (telegramAttemptFinished && !serverRunning) {

        if (webStartAt == 0) {
            webStartAt = millis();
        }

        if (millis() - webStartAt >= 1500) {
            startWebServer();
            webStartAt = 0;
        }
    }
