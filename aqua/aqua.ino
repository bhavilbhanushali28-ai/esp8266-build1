...
// ================= TELEGRAM SEND =================
if (telegramDelayActive &&
    millis() - telegramSendStartTime >= 3000 &&
    !telegramSending) {
    ...
    secureClient.stop();   // 🔴 important to clean socket
    delay(20);

    if (!secureClient.connect("api.telegram.org", 443)) {
    Serial.println(F("❌ Telegram connect failed"));
    telegramSending = false;
    telegramSendPending = false;   // prevent retry storm
    ...
    Serial.println(F("Sending message to Telegram..."));
    ...
    // 🔓 allow webserver to start (separate block)
    webServerStartPending = true;
}
