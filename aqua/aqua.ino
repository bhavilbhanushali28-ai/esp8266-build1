...
void manageWiFiConnection() {
  ...
  // 🔔 TRIGGER TELEGRAM - ALWAYS on first connection (not just final wake)
        if (!telegramTriggeredForThisConnection) {
            ...
            Serial.println(F(" Telegram scheduled"));
        }
...
void loop() {
  ...
  if (millis() - lastDebugPrint > 3000) {
    ...
    Serial.printf("webServerStartPending=%d\n", webServerStartPending);
    Serial.printf("serverRunning=%d\n", serverRunning);
  ...
  if (!bootSettled && millis() - bootStart > 3000) {
    bootSettled = true;
    Serial.println(F("✅ Boot settled"));
  }

  // ========== MID-CYCLE SLEEP (only if WiFi not needed) ==========
  if (bootSettled &&
      isMidCycleWake &&
      !deepSleepRequested &&
      rtcData.remainingSleepSeconds > 0 &&
  ...
  if (telegramDelayActive && !telegramSending) {
    ...
    if (elapsed >= 3000) {
      ...
      if (WiFi.status() != WL_CONNECTED) {
        ...
        Serial.println(F(" Sending to Telegram..."));
        ...
        // Done with Telegram (success or max retries)
        telegramSending = false;
        telegramSendPending = false;
        telegramRetryCount = 0;
        webServerStartPending = true;
        Serial.println(F("TELEGRAM: Complete, scheduling web server start"));
      }
    }
  }
  ...
  if (deepSleepRequested) {
    // Don't sleep while Telegram is busy
    if (telegramSendPending || telegramDelayActive || telegramSending) {
    ...
    if (millis() - sleepRequestTime > 300) {
      ...
      if (sleepNow == 0) {
        Serial.println(F(" Sleep duration is 0!"));
        deepSleepRequested = false;
        return;
      }
