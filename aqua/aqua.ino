...
// ========== BOOT SETTLE CHECK ==========
  ...
  // ========== MID-CYCLE SLEEP RESUME ==========
  // Only go back to sleep if:
  // 1. Boot has settled
  // 2. This is a mid-cycle wake
  // 3. WiFi is NOT connected (no point sleeping if we need to send data)
  // 4. No pending operations
  if (bootSettled &&
      isMidCycleWake &&
      !deepSleepRequested &&
      rtcData.remainingSleepSeconds > 0 &&
    ...
    Serial.println(F("Resuming chunked deep sleep (no WiFi needed)"));
  }
...
// ========== TELEGRAM HANDLING ==========
  ...
  // ========== DEEP SLEEP EXECUTION ==========
  if (deepSleepRequested) {
    // Block sleep if Telegram is active (only on final wake)
    if (!isMidCycleWake) {
      if (telegramSendPending || telegramDelayActive || telegramSending) {
        return;
      }
    ...
    if (millis() - sleepRequestTime > 300) {
      ...
      // Fresh request: load from settings
      if (sleepNow == 0) {
        sleepNow = settings.deepSleepDurationSeconds;
      }
      // Guard against zero sleep
      if (sleepNow == 0) {
        Serial.println(F("⚠️ Sleep duration is 0, aborting sleep"));
        deepSleepRequested = false;
        sleepRequestTime = 0;
        return;
      }
      ...
      if (sleepNow > 3600) {
        ...
        sleepNow = 3600;
        saveRTC();
        Serial.printf("Chunked sleep: %u sec now, %u sec remaining\n", sleepNow, rtcData.remainingSleepSeconds);
      } else {
        rtcData.remainingSleepSeconds = 0;
        clearRTC();
        Serial.printf("Final sleep chunk: %u sec\n", sleepNow);
      }
