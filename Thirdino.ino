...
if (telegramDelayActive && millis() - telegramSendStartTime >= 4000) {
  ...
  if (deepSleepRequested) {
    ...
    if (millis() - sleepRequestTime > 300) {  // Wait 300ms before sleep
      ...
      // Detach tickers (wrap each with WDT feed)
  ...
  ESP.wdtFeed();
      ...
      stopWebServer();  // Gracefully shut down server
      yield(); ESP.wdtFeed();

      WiFi.disconnect(true);  // Erase STA creds if needed
      delay(100);
      ESP.wdtFeed();

      Serial.printf("Going to deep sleep for %u seconds...\n", settings.deepSleepDurationSeconds);
      yield();
     // ESP.deepSleep(settings.deepSleepDurationSeconds * 1000000UL);  // Go to sleep
     //uint32_t sleepNow = min(remainingSleepSeconds, 3600UL); // 60 min
   /*  uint32_t sleepNow = min(remainingSleepSeconds, (uint32_t)3600);
remainingSleepSeconds -= sleepNow;

Serial.printf("Sleeping for %u seconds, remaining %u\n",
              sleepNow, remainingSleepSeconds);

ESP.deepSleep(sleepNow * 1000000UL);
*/
uint32_t sleepNow = min(remainingSleepSeconds, (uint32_t)3600); // 1 hour
remainingSleepSeconds -= sleepNow;

rtcData.remainingSleepSeconds = remainingSleepSeconds;
saveRTC();
