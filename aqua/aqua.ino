...
// ================= RTC STRUCT ==================
// This definition MUST come before the instance or functions below
struct RTCData {
  uint32_t magic;
  uint32_t remainingSleepSeconds;
  uint32_t checksum;
...
// ================= RTC INSTANCE =================
// This creates the global variable 'rtcData'
RTCData rtcData;

// ================= RTC CHECKSUM =================
// This function uses the RTCData type, so it must follow the struct
uint32_t rtcChecksum(const RTCData &d) {
  return d.magic ^ d.remainingSleepSeconds;
}
...
if (rtcValid) {
  ...
  // Sanity limit (1 day max)
  if (rtcData.remainingSleepSeconds > 86400UL) {
    Serial.println(F("⚠️ RTC corrupted (huge value). Clearing."));
    clearRTC();
  }
...
/* 🔑 MID-CYCLE WAKE FLAG (DO NOT SLEEP HERE) */
if (rtcData.remainingSleepSeconds > 0) {
  Serial.printf("MID-CYCLE WAKE: %u sec remaining\n",
                rtcData.remainingSleepSeconds);

  // 🔴 DO NOT set deepSleepRequested here
  resumeSleepAfterBoot = true;   // <-- NEW FLAG

} else {
  Serial.println(F("FINAL WAKE: Starting normal boot..."));
  resumeSleepAfterBoot = false;
}

Serial.println(F("===== SETUP END ====="));
/*  
  // --- LOAD RTC WITH SANITY CHECK ---
  bool rtcValid = loadRTC();
  if (rtcValid) {
    Serial.println(F("✅ RTC load SUCCESS"));
    // Sanity Check: If sleep time is huge (garbage), reset it
    if (rtcData.remainingSleepSeconds > 86400) { 
       Serial.println(F("⚠️ RTC data corrupted (huge value). Resetting."));
       rtcData.remainingSleepSeconds = 0;
    }
  } else {
    Serial.println(F("❌ RTC load FAILED / INVALID"));
  ...
  /* 🔑 MID-CYCLE WAKE Logic */
  if (rtcData.remainingSleepSeconds > 0) {
    Serial.printf("MID-CYCLE WAKE: %u sec remaining. Sleeping again...\n", rtcData.remainingSleepSeconds);
    deepSleepRequested = true; 
    sleepRequestTime = millis(); // Start timer immediately
  } else {
    Serial.println(F("FINAL WAKE: Starting normal boot..."));
    deepSleepRequested = false; // Ensure we stay awake
  }
...
void manageWiFiConnection() {
    ...
    // 🚫 Do NOT touch WiFi while Telegram is active
    ...
    if (!wifiConnected) {
        ...
        // 🔔 Trigger Telegram ONLY once per connection
...
void loop() {
  ...
  ESP.wdtFeed();
/*  
  // 🔁 Resume deep sleep after wake if remaining time exists
if (!deepSleepRequested && rtcData.remainingSleepSeconds > 0) {
  deepSleepRequested = true;
  sleepRequestTime = millis();  // restart delay timer
  Serial.println(F("Resuming chunked deep sleep"));
}
*/
// Allow sleep resume only after system settles
static bool bootSettled = false;
//static unsigned long bootStart = millis();

if (!bootSettled && millis() - bootStart > 5000) {
  bootSettled = true;
}

// 🔁 Resume deep sleep ONLY when safe
if (bootSettled &&
    resumeSleepAfterBoot &&
    !deepSleepRequested &&
    rtcData.remainingSleepSeconds > 0 &&
    !telegramSendPending &&
    !telegramDelayActive &&
...
if (telegramSendPending && !telegramDelayActive && !telegramSending) {
      telegramDelayActive = true;
      telegramSendStartTime = millis();
      Serial.println(F("⏳ Telegram delay started"));
  }

  if (telegramDelayActive && millis() - telegramSendStartTime >= 3000 && !telegramSending) {
      telegramDelayActive = false;
      telegramSending = true;

      Serial.println(F(" Sending Telegram..."));
      
      // REMOVED: manual secureClient.connect check (Library handles this)
      // REMOVED: secureClient.stop() (Library handles this)
      
      IPAddress ip = WiFi.localIP();
      char ipMsg[80];
      snprintf(ipMsg, sizeof(ipMsg), "ESP8266 Connected!\nIP: %d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
      ...
      telegramSending = false;
      telegramSendPending = false;
      webServerStartPending = true; // Start server after Telegram attempt
  }
...
/*
  ...
  // 🚫 DO NOT SLEEP WHILE TELEGRAM IS ACTIVE
  if (telegramSendPending || telegramDelayActive || telegramSending) {
    return;
  }
...
*/
 // Trigger sleep ONLY if requested AND Telegram is not busy
  if (deepSleepRequested) {
      // 🛡️ GUARD: If this is a FINAL wake (normal boot), DO NOT SLEEP until Telegram is done!
      // If remainingSleepSeconds > 0, it's a mid-cycle wake, so we ignore Telegram and sleep.
      if (rtcData.remainingSleepSeconds == 0) {
          if (telegramSendPending || telegramDelayActive || telegramSending) {
              return; // ⛔ Wait for Telegram to finish
          }
      }

      if (sleepRequestTime == 0) sleepRequestTime = millis();

      if (millis() - sleepRequestTime > 300) { 
          Serial.println(F("Entering Deep Sleep..."));
          ESP.wdtFeed();
            // Detach tickers
    failsafeTicker.detach();        ESP.wdtFeed();
    ...
    deferredServerDelete.detach();  ESP.wdtFeed();
          ...
          // If this was a fresh request (seconds == 0), load from settings
          if (sleepNow == 0) {
             sleepNow = settings.deepSleepDurationSeconds;
          }

          // Chunk logic: Cap at 1 hour (3600s)
          if (sleepNow > 3600) {
             rtcData.remainingSleepSeconds = sleepNow - 3600;
             sleepNow = 3600;
             saveRTC(); // Save remaining time for next wake
          } else {
             rtcData.remainingSleepSeconds = 0;
             clearRTC(); // Clear RTC for next full boot
          }
