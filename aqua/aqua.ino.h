...
volatile bool shouldLogRelayFlag      = false;  // ADD
volatile bool shouldLogFailsafeFlag   = false;  // ADD
...
*/
bool acquireFSLock(unsigned long timeout = 0) {  // timeout unused now
  if (fsLock) return false;   // don't wait, just fail immediately
  fsLock = true;
  return true;
}
void releaseFSLock() {
  ...
  if (strcmp(state, "on") == 0 || strcmp(state, "toggle") == 0) {
    if (relayState) {
      if (strcmp(state, "toggle") == 0) {
        ...
        if (failsafeTicker.active()) failsafeTicker.detach();  // FIX: also detach failsafe
      ...
      // FIX: detach both before re-arming
      if (relayTicker.active()) relayTicker.detach();
      ...
      if (settings.failsafeEnabled && settings.maxRelayRuntime > 0)
        failsafeTicker.once(settings.maxRelayRuntime, failsafeTriggered);
...
bool hardcodedSSIDAvailable(const char* ssid) {
  int n = WiFi.scanComplete();
  if (n <= 0) return false;  // no scan results yet
  for (int i = 0; i < n; i++) {
...
void manageWiFiConnection() {
    ...
    if (!wifiConnected) {
        ...
        if (needsSave) {
    shouldSaveSettingsFlag = true;
    Serial.println(F("Network credentials will be saved."));
}
...
void loop() {
  ...
  // Mid-cycle deep sleep wake — handle and return immediately
  ...
  // ── SWITCH: always first, always fast ──
  checkSwitchPress();
  ...
  if (millis() - lastEmergencyCheck > 10) {
    ...
    if (lastEmergencyState == HIGH && currentEmergencyState == LOW) {
      strncpy(pendingFailsafeReason, "Button:EmergencyStop", sizeof(pendingFailsafeReason) - 1);
  ...
  // ── DEFERRED FS WRITES — one per loop, prioritised ──
  // FIX 2: removed duplicate shouldSaveSettingsFlag block that was at line 1208
  // FIX 3: relayTimedOffPending now sets flags instead of calling directly
  if (shouldLogFailsafeFlag) {
    shouldLogFailsafeFlag = false;
    if (pendingFailsafeReason[0]) {
      logFailsafeEvent(pendingFailsafeReason);
      pendingFailsafeReason[0] = 0;
    }
  } else if (shouldLogRelayFlag) {
    shouldLogRelayFlag = false;
    logRelayRunTime();
  } else if (shouldSaveSettingsFlag) {
    shouldSaveSettingsFlag = false;
    saveSettings();
  }

  // ── RELAY TIMED OFF (set by Ticker ISR) ──
  if (relayTimedOffPending) {
    ...
    if (relayState) {
      ...
      if (relayTicker.active()) relayTicker.detach();
      if (failsafeTicker.active()) failsafeTicker.detach();
      unsigned long actualDuration = (millis() - relayOnStartTime) / 1000UL;
      Serial.printf("Relay OFF after %lu seconds.\n", actualDuration);
      if (!isTemporaryRun) {
        // FIX 3: use flags, not direct calls
        shouldLogRelayFlag = true;
        shouldSaveSettingsFlag = true;
      } else {
  ...
  if (millis() - lastHeapCheck > 5000) {
    ...
    if (ESP.getFreeHeap() < 4000) {
      Serial.println(F("CRITICAL: Heap very low!"));
    }
  }
  ...
  if (telegramDelayActive && !telegramSending) {
    if (millis() - telegramSendStartTime >= 3000) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.println(ok ? F("Telegram sent.") : F("Telegram failed."));
      }
      telegramSending = false;
      telegramSendPending = false;
      webServerStartPending = true;
  ...
  if (webServerStartPending && !serverRunning) {
    if (!telegramSendPending && !telegramDelayActive && !telegramSending) {
      ...
      // FIX: removed delay(100) — was blocking the loop
    }
  }

  if (deepSleepRequested) {
    if (telegramSendPending || telegramDelayActive || telegramSending) return;
    ...
    if (millis() - sleepRequestTime > 500) {
      ...
      if (serverRunning) { stopWebServer(); serverRunning = false; }
      WiFi.mode(WIFI_OFF);
      delay(100);
  ...
  if (WiFi.status() != WL_CONNECTED) scanDone = false;

  // Debug print — moved to end, low frequency
  static unsigned long lastDebug = 0;
  if (millis() - lastDebug > 30000) {
    lastDebug = millis();
    Serial.printf("WiFi:%d heap:%u relay:%d\n",
                  WiFi.status(), ESP.getFreeHeap(), relayState);
  }
}
void safeRestart() {
...
void applyDefaultSettings() {
  ...
  settings.autoDeepSleepDelayAfterRun = 10;  // 10 seconds (was bool default = 0/false)
}
void setupLittleFS() {
  if (!LittleFS.begin()) {
#ifdef DEBUG_MODE
...
void checkSwitchPress() {
  ...
  // Only proceed once the reading has been stable for DEBOUNCE_DELAY
  if ((millis() - lastSwitchDebounceTime) < DEBOUNCE_DELAY) {
    return;
  }

  // --- cooldown guard ---
  bool cooldownOk = (millis() - lastSwitchActionTime) >= SWITCH_CHANGE_COOLDOWN;

  // --- act on FALLING edge (LOW = pressed) to turn relay ON ---
  if (currentReading == LOW && lastSwitchStableState == HIGH) {
    // stable transition: released → pressed
    lastSwitchStableState = LOW;
    if (cooldownOk) {
      if (!relayState) {
        Serial.println(F("Switch pressed → Relay ON"));
        lastSwitchActionTime = millis();
        toggleRelayInternal();
      } else {
        Serial.println(F("Switch pressed but relay already ON – ignored"));
        lastSwitchActionTime = millis(); // reset cooldown to prevent rapid re-fire
      }
    } else {
      Serial.println(F("Switch pressed but cooldown active"));
    }
  }

  // --- act on RISING edge (HIGH = released) to turn relay OFF ---
  else if (currentReading == HIGH && lastSwitchStableState == LOW) {
    // stable transition: pressed → released
    lastSwitchStableState = HIGH;
    if (cooldownOk) {
      if (relayState) {
        Serial.println(F("Switch released → Relay OFF"));
        lastSwitchActionTime = millis();
        toggleRelayInternal();
      } else {
        Serial.println(F("Switch released but relay already OFF – ignored"));
        lastSwitchActionTime = millis();
      }
    } else {
      Serial.println(F("Switch released but cooldown active"));
    }
  }
}
const char* getContentType(const char* filename) {
...
bool saveSettings() {
  bool success = false;                              // ← was deleted, restored
  File configFile = LittleFS.open("/settings.tmp", "w");  // ← was deleted, restored
  if (!configFile) {
    Serial.println(F("Failed to open temp settings file."));
    return false;
  }
  ESP.wdtFeed();
  StaticJsonDocument<SETTINGS_DOC_CAPACITY> doc;
  ...
  if (serializeJson(doc, configFile) == 0) {
    Serial.println(F("Failed to serialize settings JSON."));
    configFile.close();
    return false;
  }
  configFile.close();
  ...
  if (success) {
    ...
    Serial.println(F("Failed to rename settings file."));
  }
  return success;
}
bool loadBootRecovery() {
...
void toggleRelayInternal() {
  if (failsafeActive || emergencyStop) {
    Serial.println(F("Cannot toggle relay: Failsafe or Emergency Stop active."));
    return;
  }
  ...
  if (relayState) {
    ...
    digitalWrite(settings.relayPin, HIGH);
    relayState = false;
    Serial.println(F("Relay OFF (switch toggle)."));
    ...
    // Non-blocking: let the main loop write to flash
    shouldSaveSettingsFlag = true;
    ...
    // ── TURN ON ──
    // Always detach any stale tickers first
    if (relayTicker.active()) relayTicker.detach();
    if (failsafeTicker.active()) failsafeTicker.detach();
    ...
    digitalWrite(settings.relayPin, LOW);
    relayState = true;
    Serial.printf_P(PSTR("Relay ON (switch, timed %lu sec).\n"), relayDuration);

    if (relayDuration > 0) {
      // .once() fires exactly once after `relayDuration` seconds
      relayTicker.once(relayDuration, turnOffRelayTimed);
    }

    if (settings.failsafeEnabled && settings.maxRelayRuntime > 0) {
      // FIX: use .once(), NOT .attach() – failsafe should fire once only
      failsafeTicker.once(settings.maxRelayRuntime, failsafeTriggered);
    }
